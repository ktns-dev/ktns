...
import { Search, ChevronLeft, ChevronRight, LoaderIcon, Clock, Calendar } from "lucide-react";
  ...
  const columns: ColumnDef<ClassTiming>[] = [
    {
      ...
      cell: ({ row }) => (
        <div className="font-semibold text-slate-500 dark:text-slate-400">
          #{row.getValue("attendance_time_id")}
        </div>
      ),
    },
    {
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        ...
        <div className="px-5 pt-5 pb-4 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-semibold text-indigo-500 uppercase tracking-widest mb-0.5">Master Data</p>
              <h2 className="text-base font-bold text-slate-800 dark:text-slate-100">Class Timings</h2>
            </div>
          </div>
              ...
              {loading ? (
                <TableRow>
                  <TableCell colSpan={columns.length} className="text-center py-12">
                    ...
                    <p className="mt-2 text-sm text-slate-500">Loading timings...</p>
                  </TableCell>
                </TableRow>
            ...
            table.getRowModel().rows.map((row) => (
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-xs text-slate-400 font-medium tracking-wide">#{row.original.attendance_time_id}</p>
